      ...
      >
        <div className="flex items-start gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <span className="citation-marker text-[10px] text-green/70 tracking-wide">SOURCE {String(index + 1).padStart(2, '0')} · {source.jurisdiction}</span>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <span className="citation-marker text-xs text-gold-dark bg-gold-light/15 border border-gold/20 px-2 py-1 rounded">{Math.round(source.relevance_score * 100)}% match</span>
      ...
      {open && (
        <div className="px-4 sm:px-5 pb-5 pt-1 border-t border-hairline bg-paper/45 animate-in">
          <dl className="grid sm:grid-cols-2 gap-x-6 gap-y-2 text-xs text-ink/70 mt-3">
            <div><dt className="inline font-medium text-ink/80">Authority: </dt><dd className="inline">{source.authority}</dd></div>
            <div><dt className="inline font-medium text-ink/80">Domain: </dt><dd className="inline">{source.domain}</dd></div>
            <div><dt className="inline font-medium text-ink/80">Source type: </dt><dd className="inline">{source.source_type}</dd></div>
            <div><dt className="inline font-medium text-ink/80">Version: </dt><dd className="inline">{source.version_date}</dd></div>
            <div><dt className="inline font-medium text-ink/80">Retrieved: </dt><dd className="inline">{source.retrieved_date}</dd></div>
            <div className="sm:col-span-2"><dt className="inline font-medium text-ink/80">Precision: </dt><dd className="inline">{source.precision}</dd></div>
          </dl>
          {isValidUrl ? (
            <a
              href={url}
